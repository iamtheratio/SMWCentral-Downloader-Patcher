    ...
    for i, hack in enumerate(selected_hacks, 1):
        ...
        if hack_id in processed:
            ...
            if existing_hack.get("current_difficulty") != display_diff:
                ...
                # Move file to new difficulty folder if it exists
                old_file_path = existing_hack.get("file_path")
                if old_file_path and os.path.exists(old_file_path):
                    # Calculate new path based on new difficulty
                    filename = os.path.basename(old_file_path)
                    new_file_path = os.path.join(make_output_path(output_dir, primary_type, folder_name), filename)
                    ...
                    if old_file_path != new_file_path:
                        try:
                            ...
                            existing_hack["file_path"] = new_file_path
                            if log:
                                log(f"📁 Moved file from {os.path.dirname(old_file_path)} to {os.path.dirname(new_file_path)}", "Information")
                            ...
                            if log:
                                log(f"⚠️ Failed to move file to new difficulty folder: {str(e)}", "Warning")
