...
class UploadManagerV2Simple:
    ...
    async def upload_multiple_files(self, file_pairs: list) -> dict:
        ...
        """
        try:
            # Validate local file (V1 pattern)
            if not os.path.exists(local_path):
                self.log_error(f"Local file not found: {local_path}")
                return False
            ...
            file_size = os.path.getsize(local_path)
            if file_size == 0:
                self.log_error(f"Cannot upload empty file: {local_path}")
                return False
            
            self.log_progress(f"📤 Uploading {os.path.basename(local_path)} ({file_size} bytes)")
            
            # Ensure remote directory exists using V2 filesystem
            remote_dir = str(Path(remote_path).parent)
            if remote_dir != "/" and remote_dir != ".":
                if not await self.filesystem.ensure_directory(remote_dir):
                    self.log_error(f"Failed to ensure directory: {remote_dir}")
                    return False
            
            # Upload using proven V1 pattern
            return await self._upload_with_v1_pattern(local_path, remote_path, file_size)
            
        except Exception as e:
            self.log_error(f"Upload failed: {str(e)}")
            return False
    
    async def _upload_with_v1_pattern(self, local_path: str, remote_path: str, file_size: int) -> bool:
        """
        Core upload logic using exact V1 pattern
        This is the proven working approach from qusb2snes_sync.py
        """
        try:
            # V1 pattern: Ensure connection is stable before upload
            if not self.connection.is_connected():
                self.log_error("Connection lost before upload")
                return False
            
            # Step 1: Send PutFile command (V1 pattern)
            size_hex = format(file_size, 'X')
            
            # Use V2 connection but with V1 command pattern
            # PutFile is a no-reply command, so expect_response=False
            response = await self.connection.send_command(
                "PutFile", 
                operands=[remote_path, size_hex], 
            ...
            # V1 pattern: Check for None (command failure)
            if response is None:
                raise Exception(f"PutFile command failed for {remote_path}")
            
            self.log_progress("✅ PutFile command sent successfully")
            
            # Step 2: V1 proven delay for SD card preparation
            await asyncio.sleep(self.sd_card_prep_delay)
            
            # Step 3: Send file data using V1 chunk pattern
            return await self._send_file_data_v1_pattern(local_path, file_size)
            
        except Exception as e:
            self.log_error(f"V1 pattern upload failed: {str(e)}")
            return False
    
    async def _send_file_data_v1_pattern(self, local_path: str, file_size: int) -> bool:
        """
        Send file data using exact V1 proven pattern
        """
        bytes_sent = 0
        total_chunks = 0
        ...
        try:
            with open(local_path, "rb") as f:
                while bytes_sent < file_size:
                    # V1 pattern: Check connection before each chunk
                    if not self.connection.is_connected() or not self.connection.websocket:
                        raise Exception("Connection lost during file transfer")
                    
                    # V1 pattern: Read chunk directly from file
                    chunk = f.read(min(self.chunk_size, file_size - bytes_sent))
                    if not chunk:
                        break
                    
                    try:
                        # V1 pattern: Direct WebSocket send
                        await self.connection.websocket.send(chunk)
                        bytes_sent += len(chunk)
                        total_chunks += 1
                        
                        # V1 pattern: Progress feedback every 500KB
                        if bytes_sent % self.progress_report_interval == 0 or bytes_sent == file_size:
                            progress_kb = bytes_sent // 1024
                            total_kb = file_size // 1024
                            progress_percent = (bytes_sent / file_size) * 100
                            self.log_progress(f"📤 Upload progress: {progress_percent:.1f}% ({progress_kb}KB/{total_kb}KB)")
                        
                        # V1 pattern: Critical timing for SD card compatibility
                        if bytes_sent < file_size:
                            if bytes_sent % (64 * 1024) == 0:
                                # V1 pattern: Longer pause every 64KB for SD card
                                await asyncio.sleep(self.sd_card_pause_delay)
                            else:
                                # V1 pattern: Regular delay
                                await asyncio.sleep(self.regular_delay)
                                
                    except Exception as e:
                        raise Exception(f"Failed to send chunk at byte {bytes_sent}: {str(e)}")
            
            # V1 pattern: Verify all bytes were sent
            if bytes_sent != file_size:
                raise Exception(f"Incomplete upload: {bytes_sent}/{file_size} bytes sent")
            
            self.log_progress(f"✅ Upload completed: {bytes_sent} bytes in {total_chunks} chunks")
            return True
            
        except Exception as e:
            self.log_error(f"File data transmission failed: {str(e)}")
            return False
    
    async def cleanup(self):
        """Clean up resources (no complex state to clean)"""
        self.log_progress("Upload Manager V2 cleanup completed")